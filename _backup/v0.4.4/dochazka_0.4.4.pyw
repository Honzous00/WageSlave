...
import threading
from collections import defaultdict   # <-- přidáno pro měsíční konto
...
class PatecniVestec:
    ...
    def obnovit_data(self):
        ...
        if filtr == "Aktuální týden":
            ...
            # --- MĚSÍČNÍ KONTO (sčítání více záznamů za den) ---
            ...
            uzavrene_mesic = defaultdict(int)   # datum -> součet minut z uzavřených
            otevrene_mesic = {}                  # datum -> záznam (poslední otevřený)
            ...
            while den <= dnes:
                if den.weekday() < 5:  # pracovní den
                    ...
                    # přičteme uzavřené záznamy
                    skutecne_mesic += uzavrene_mesic.get(datum_str, 0)
                    # pokud je dnešek a existuje otevřený záznam, připočteme aktuální čas
                    if datum_str == dnes.strftime("%Y-%m-%d") and datum_str in otevrene_mesic:
                        ...
                        if prichod:
                            ...
                            if ted > p_dt:
                                ...
                                if diff > 360:
                                    diff -= OBED_MINUT
                                skutecne_mesic += max(0, diff)
                den += timedelta(days=1)

            konto = skutecne_mesic - fond_mesic
            barva = "#00cc66" if konto >= 0 else "#ff4444"
            self.lbl_konto.config(text=f"Měsíční konto: {formatuj_minuty(abs(konto))}", fg=barva)

        else:
            self.lbl_patek.config(text="--:--")
            self.lbl_stats.config(text="Odpracováno: --")
            self.lbl_remaining.config(text="Zbývá: --")
            self.lbl_konto.config(text="Měsíční konto: --", fg="white")
