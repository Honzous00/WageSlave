...
class PatecniVestec:
    ...
    def create_widgets(self):
        ...
        # --- VSTUPNÍ KARTA ---
        entry_frame = ttk.LabelFrame(self.main_container, text=" ZÁZNAM ", padding=15)
        ...
        prichod_frame = tk.Frame(entry_frame, bg="#2d2d2d")
        prichod_frame.grid(row=1, column=1, columnspan=2, sticky='w')
        
        self.ent_prichod = ttk.Entry(prichod_frame, width=8)
        self.ent_prichod.pack(side=tk.LEFT)
        ttk.Button(prichod_frame, text="TEĎ", width=5,
                   command=lambda: self.nastav_cas(self.ent_prichod)).pack(side=tk.LEFT, padx=5)

        # Řádek 2: Odchod
        ttk.Label(entry_frame, text="Odchod:").grid(row=2, column=0, **grid_params)
        odchod_frame = tk.Frame(entry_frame, bg="#2d2d2d")
        odchod_frame.grid(row=2, column=1, columnspan=2, sticky='w')
        
        self.ent_odchod = ttk.Entry(odchod_frame, width=8)
        self.ent_odchod.pack(side=tk.LEFT)
        ttk.Button(odchod_frame, text="TEĎ", width=5,
                   command=lambda: self.nastav_cas(self.ent_odchod)).pack(side=tk.LEFT, padx=5)
        ...
        # Tlačítko Uložit (samostatné, celou šířkou)
        self.btn_ulozit = ttk.Button(entry_frame, text="ULOŽIT ZÁZNAM", command=self.ulozit)
        self.btn_ulozit.grid(row=4, column=0, columnspan=3, sticky='ew', pady=5)

        # --- HISTORIE ---
        history_frame = ttk.LabelFrame(self.main_container, text=" HISTORIE ", padding=10)
        history_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        # Filtr týdnů (vlevo), tlačítko smazat (vpravo)
    ...
    def obnovit_data(self):
        ...
        if filtr == "Aktuální týden":
            # Zjistíme, které dny v týdnu máme zaznamenány
            dny_v_tydnu = {}
            for r in vsechny_tyden:
                datum = datetime.strptime(r[2], "%Y-%m-%d")
                den_v_tydnu = datum.weekday()  # 0=Po, 4=Pá
                dny_v_tydnu[den_v_tydnu] = {
                    'prichod': r[3],
                    'odchod': r[4],
                    'minut': r[5] if r[4] else None
                }

            # Spočítáme skutečně odpracované minuty z existujících záznamů
            skutecne_odpracovano = sum(r[5] for r in vsechny_tyden if r[4])  # jen dokončené dny
            
            # Připočteme dnešek, pokud je "V PRÁCI"
            dnes_v_tydnu = ted.weekday()
            if dnes_v_tydnu in dny_v_tydnu and not dny_v_tydnu[dnes_v_tydnu]['odchod']:
                try:
                    p_dt = ted.replace(hour=datetime.strptime(dny_v_tydnu[dnes_v_tydnu]['prichod'], "%H:%M").hour,
                                       minute=datetime.strptime(dny_v_tydnu[dnes_v_tydnu]['prichod'], "%H:%M").minute)
                    if ted > p_dt:
                        diff = int((ted - p_dt).total_seconds() / 60)
                        if diff > 360:
                            diff -= OBED_MINUT
                        skutecne_odpracovano += max(0, diff)
                except:
                    pass

            # Spočítáme ideální stav k dnešku (bez pátku)
            ideal_k_dnes = self.vypocitej_idealni_odpracovano(hledany_tyden, dnes_v_tydnu)
            
            # Rozdíl oproti ideálu
            rozdil = skutecne_odpracovano - ideal_k_dnes
            
            # Kolik musíme odpracovat v pátek?
            # Standardně v pátek 8h (480 min), ale upravíme o rozdíl
            patkovy_fond = 480 - rozdil
            
            # Výpočet odchodu v pátek (příchod v 7:00 + patkovy_fond + oběd)
            prichod_patek_min = cas_na_minuty("07:00")
            obed_patek = OBED_MINUT
            odchod_patek_min = prichod_patek_min + patkovy_fond + obed_patek
            
            # Převedeme na čas
            if odchod_patek_min < 24*60:
                hodiny = odchod_patek_min // 60
                minuty = odchod_patek_min % 60
                cas_patek = f"{hodiny:02d}:{minuty:02d}"
                
                # Kontrola, zda už máme páteční záznam
                if 4 in dny_v_tydnu and dny_v_tydnu[4]['odchod']:
                    cas_patek = f"✓ {dny_v_tydnu[4]['odchod']} (hotovo)"
            else:
                cas_patek = "NESTÍHÁŠ!"
            ...
            procenta = min(100, int((skutecne_odpracovano / celkem_minut) * 100)) if celkem_minut else 0
            ...
            self.lbl_remaining.config(text=f"Zbývá: {formatuj_minuty(celkem_minut - skutecne_odpracovano)}")
