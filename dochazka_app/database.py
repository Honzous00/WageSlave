...
class DochazkaDB:
    ...
    def insert(self, datum, prichod, odchod, obed, tyden):
        with self._connect() as conn:
            cursor = conn.execute(
                ...
                (tyden, datum, prichod, odchod,
                 0 if not odchod else max(0, self._vypocti_minuty(prichod, odchod, obed)), obed)
            )
    ...
    def update(self, id, datum, prichod, odchod, obed, tyden):
        with self._connect() as conn:
            conn.execute(
                ...
                (tyden, datum, prichod, odchod,
                 0 if not odchod else max(0, self._vypocti_minuty(prichod, odchod, obed)), obed, id)
            )
    ...
    def fetch_by_week(self, tyden):
        with self._connect() as conn:
            return conn.execute(
                "SELECT * FROM dochazka WHERE tyden=? ORDER BY datum DESC", (tyden,)
            ).fetchall()

    def fetch_by_month(self, month_str):
        with self._connect() as conn:
            return conn.execute(
    ...
    def fetch_distinct_weeks(self):
        with self._connect() as conn:
            return [row[0] for row in conn.execute(
                "SELECT DISTINCT tyden FROM dochazka ORDER BY tyden DESC"
            ).fetchall()]
    ...
    def fetch_for_export(self, months):
        ...
        with self._connect() as conn:
            return conn.execute(
                f"SELECT datum, prichod, odchod FROM dochazka "
                f"WHERE substr(datum,1,7) IN ({placeholders}) ORDER BY datum ASC",
                months
            ).fetchall()
    ...
    def _vypocti_minuty(self, prichod, odchod, obed):
        ...
        return int((t2 - t1).total_seconds() / 60) - (config.OBED_MINUT if obed else 0)
    ...
    def update_odchod(self, record_id, odchod):
        with self._connect() as conn:
            cur = conn.execute("SELECT prichod, obed FROM dochazka WHERE id=?", (record_id,))
            row = cur.fetchone()
            if not row:
                return
            prichod, obed = row
